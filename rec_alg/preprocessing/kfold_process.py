...
class KFoldProcess(BaseProcess):
    ...
    def transform(self, sep="\t", chunksize=10000, shuffle=True, mode="fast"):
        counter = 0
        ...
        for file_path in self.train_files:
            ...
            for n, data_chunk in enumerate(iterator):
                ...
                self._save_fold_data(folds, data_chunk, filename, sep, mode)
                print("KFoldProcess::transform: save train_data done")
        return True
    
    def _save_fold_data(self, folds, train_data, filename, sep="\t", mode="fast"):
        for i in range(len(folds)):
            ...
            with open(abs_filename, "a", encoding="utf-8") as f:
                if mode == "fast":
                    fold_data = train_data.values[valid_id]
                    for data in fold_data:
                        f.write(sep.join([str(item) for item in data]) + "\n")
                else:
